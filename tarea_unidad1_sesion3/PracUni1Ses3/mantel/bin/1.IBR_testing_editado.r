...
# Realizar Partial Mantel test entre Fst y las matrices del presente y LGM,
# parcializando la matriz flat (controlando por el efecto de la resistencia plana)

## Partial Mantel test: Present vs Fst, parcializando flat
print("Partial Mantel test: Present vs Fst (controlando por flat)")
partial_present <- mantel.partial(as.dist(B.present), 
                                  as.dist(B.FstLin), 
                                  as.dist(B.flat),
                                  method = "pearson",
                                  permutations = 10000)
print(partial_present)

## Partial Mantel test: CCSM (LGM) vs Fst, parcializando flat
print("Partial Mantel test: CCSM vs Fst (controlando por flat)")
partial_ccsm <- mantel.partial(as.dist(B.ccsm), 
                               as.dist(B.FstLin), 
                               as.dist(B.flat),
                               method = "pearson",
                               permutations = 10000)
...
## Partial Mantel test: MIROC (LGM) vs Fst, parcializando flat
print("Partial Mantel test: MIROC vs Fst (controlando por flat)")
partial_miroc <- mantel.partial(as.dist(B.miroc), 
                                as.dist(B.FstLin), 
                                as.dist(B.flat),
                                method = "pearson",
                                permutations = 10000)
...
# Crear un dataframe con los resultados de los Partial Mantel tests
partial_results <- data.frame(
  Scenario = c("Present", "CCSM", "MIROC"),
  Statistic = c(partial_present$statistic, partial_ccsm$statistic, partial_miroc$statistic),
  Significance = c(partial_present$signif, partial_ccsm$signif, partial_miroc$signif),
  P_value = c(partial_present$signif, partial_ccsm$signif, partial_miroc$signif)
)

print("Resumen de Partial Mantel Tests:")
